...
<script>
// 當 class 為 .tag 的元素被點擊時觸發此函式
$(".tag").on("click", function() {
    
    // 1. 取得目前被點擊的標籤 (this) 在同級元素中的順序索引值 (從 0 開始)
    let index = $(this).index();
    
    // 2. 移除所有 .post (內容區塊) 的 .active 樣式，將所有內容隱藏
    $(".post").removeClass('active');
    
    // 3. 移除所有 .tag (標籤) 的 .active 樣式，重置標籤狀態
    $(".tag").removeClass("active");
    
    // 4. 選取第 index 個 .post 區塊，並加上 .active 樣式使其顯示
    $(".post").eq(index).addClass("active");
    
    // (已註解) //$(".tag").eq(index).addClass("active") 
    // 這是原本嘗試透過 index 來啟用標籤樣式的寫法，現在改用更簡潔的方式
    
    // 5. 為當前被點擊的標籤 (this) 加上 .active 樣式，標記為「目前選中」狀態
...
<!--
邏輯流程總結：
監聽事件：當使用者點擊任何一個具有 .tag 類別的元素時，觸發監聽函式。

定位目標：透過 $(this).index() 抓到使用者點擊的是「第幾個」標籤。

重置狀態：先把原本所有顯示中的標籤與內容的「啟用樣式 (active)」全部移除，確保不會有多個區塊同時重疊。

同步顯示：使用 eq(index) 準確鎖定對應的內容區塊與標籤，並賦予它們顯示的樣式。

小提示：
這段程式碼非常簡潔且具備通用性，
前提是你的 HTML 結構中 .tag 與 .post 的順序必須是一一對應的
（例如第 1 個 tag 對應第 1 個 post）。
-->